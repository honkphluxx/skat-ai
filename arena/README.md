...
```
ramsch                        0.00%        0.00%
External bots: 900 games, 124 delegated (13,78%), 0 rule divergences
```
...
work. It does. `startGame` arrives from inside the engine's
`restartWithDeal`, which runs the auction *and* opens trick play in one call --
...
alternative, moving the void check before `startGame`, would mean teaching the
engine a rule the app does not play, which is the thing this arena exists not
to do.

Two smaller things from the same three lines. `13,78%` -- the comma is a German
locale, and every other number this arena prints goes through `Locale.ROOT`
...
And `900 games` for a 600-game match is right, not a bug: a seat is counted, and
